...
public class PatientProfileService {
    ...
    // Thêm hàm trả về toàn bộ steps của mọi booking cho bệnh nhân
    public List<BookingStep> getAllStepsOfPatient(Integer cusId) {
        List<Booking> bookings = bookingRepository.findByCusIdOrderByBookIdDesc(cusId);
        if (bookings.isEmpty()) return List.of();

        return bookings.stream()
                .flatMap(booking -> bookingStepRepository.findByBookId(booking.getBookId()).stream())
