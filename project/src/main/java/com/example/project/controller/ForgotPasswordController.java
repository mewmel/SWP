...
public class ForgotPasswordController {
    ...
    /**
     * API test để lấy OTP mà không gửi email (chỉ để debug)
     * POST /api/forgot-password/test-otp
     */
    @PostMapping("/test-otp")
    public ResponseEntity<Map<String, Object>> testOtp(@RequestBody Map<String, String> request) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            // Tạo OTP và lưu vào cache (không gửi email)
            String otp = generateTestOtp();
            // Lưu OTP vào cache để verify sau này
            otpService.saveOtpForTesting(customer.getCusId(), otp);
            
            response.put("success", true);
            response.put("message", "OTP đã được tạo thành công! (Test mode - không gửi email)");
            response.put("otp", otp); // Trả về OTP để test
            response.put("customerId", customer.getCusId());
            ...
            e.printStackTrace();
            response.put("success", false);
            response.put("message", "Lỗi khi tạo OTP test: " + e.getMessage());
            return ResponseEntity.status(500).body(response);
        }
    }
