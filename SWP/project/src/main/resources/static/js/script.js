document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('loginForm').addEventListener('submit', function(e) {
        ...
        fetch('/api/auth/login', {
            ...
            .then(data => {
                ...
                localStorage.setItem('userEmail', data.cusEmail || 'Chưa cập nhật email');
                localStorage.setItem('userPhone', data.cusPhone || 'Chưa cập nhật số điện thoại');
                localStorage.setItem('userDob', data.cusDate || 'Chưa cập nhật ngày sinh');
    ...
    document.getElementById('registerForm').addEventListener('submit', function(e) {
        ...
        const cusPhone = document.getElementById('registerPhone').value;
        const cusDate = document.getElementById('registerDob').value;
        ...
        fetch('/api/auth/register', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ cusFullName, cusEmail, cusPhone, cusDate, cusPassword, confirmPassword })
        })
            .then(async response => {
            ...
            .catch(err => {
                ...
                } else if (message.includes('Bạn phải đủ 18 tuổi để đăng ký!')) {
                    showNotification(message, 'error');
                } else if (message.includes('Số điện thoại phải gồm đúng 10 chữ số!')) {
                    showNotification(message, 'error');
                } else {
                    showNotification(message, 'error');
                }
